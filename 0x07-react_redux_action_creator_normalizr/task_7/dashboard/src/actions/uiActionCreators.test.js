...
import { login, loginSuccess, loginFailure } from "./uiActionCreators";
...
describe("Testing uiActionCreators", () => {
  it("test login action creator", () => {
    ...
    expect(uiActionCreators.login("example@example.com", "samplePass")).toEqual(
      expected
    );
  });
...
describe("test loginRequest action", () => {
  it("verify that if the API returns the right response, the store received two actions LOGIN and LOGING_SUCCESS", () => {
    ...
    const fetch_mock = fetchMock.mock(
      "http://localhost:8564/login-success.json",
      200
    );

    return store
      .dispatch(uiActionCreators.loginRequest("test@test", "test"))
      .then(() => {
